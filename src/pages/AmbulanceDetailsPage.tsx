...
export function AmbulanceDetailsPage() {
  ...
  const submitReview = async () => {
    if (!ambulance) return;
    if (newRating < 1 || newRating > 5 || !newReview.trim()) {
      alert('Please give a rating and write a review');
      return;
    }
  
    try {
      const res = await api.post('/api/v1/ambulance/create-ambulance-review', {
      ...
      if (res.data.code === 'XS0001') {
        ...
        setNewRating(0);
        setNewReview('');
        fetchAmbulanceDetails(ambulance.id.toString()); // Refresh everything including reviews + eligibility
      } else {
  ...
  const fetchAmbulanceDetails = async (ambulanceId: string) => {
    try {
      ...
      if (baseResponse.code === 'XS0001') {
        ...
        const eligibility = await api.get(`/api/v1/ambulance/can-review/${ambulanceId}`);
        if (eligibility.data.code === 'XS0001') {
          setCanReview(!!eligibility.data.data);
        }
      }
    } catch (error) {
      console.error('Failed to fetch ambulance details or eligibility', error);
    } finally {
      setLoading(false);
    }
  };
